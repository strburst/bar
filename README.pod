...
Set the underline width in pixels. The default is 1.
...
lemonbar provides a screenrc-inspired formatting syntax to allow full customization at runtime. Every formatting block is opened with B<%{> and closed with B<}> and accepts the following commands. The parser tries its best to handle malformed input.
...
Align the following text to the left side of the screen.

=item B<c>

Align the following text to the center of the screen.

=item B<r>

Align the following text to the right side of the screen.
...
The I<button> field is optional. It defaults to the left button, and is a number ranging from 1 to 5 which maps to the left, middle, right, scroll up and scroll down movements. Your mileage may vary.
...
Support for clickable areas was heavily based off a u-ra contribution
